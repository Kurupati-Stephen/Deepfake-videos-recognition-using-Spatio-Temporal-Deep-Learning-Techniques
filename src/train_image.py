...
def train_image_model():
    ...
    train_loader, test_loader = get_image_dataloaders(config)
    ...
    print(f"Image Dataset Split: {num_neg} Authentic, {num_pos} Synthetic")
    ...
    for epoch in range(epochs):
        ...
        for batch_imgs, batch_labels in tqdm(train_loader, desc=f"Epoch {epoch+1}/{epochs}"):
        ...
        print(f"Epoch {epoch+1}/{epochs}, Image Loss: {avg_loss:.4f}")
        
        scheduler.step(avg_loss)
        
        if avg_loss < best_loss:
            best_loss = avg_loss
            torch.save(model.state_dict(), os.path.join(models_dir, "best_image_model.pth"))
            patience_counter = 0
        else:
            patience_counter += 1
